...
public class Token {
    ...
    Token(long mac, long tokenTime, int pid, int sequence, String clientIp) {
		super();
		this.mac = mac;
        ...
        for (int i = DATA_LENGTH, j = 0; i < chs.length; i++, j++) {
            if (chs[i] != hash[j]) {
                log.warn("value hash is invalid, value hash: [" + new String(chs, DATA_LENGTH, TOKEN_LENGTH - DATA_LENGTH) +
                		"], calculate hash: [" + new String(chs) + "], token: [" + value + "]");
                return false;
            }
        }
        return true;
    ...
    public String toCookieString() {
        ...
        offset = encodeNode(chs, p++, ServletUtil.parseIp(clientIp), offset, 6, 5, 10000000000L);
    ...
    static long decodeNode(char[] chs, int offset, int len, long randomBase) {
        long n = 0;
	...
	static String toIpAddr(long ip) {
