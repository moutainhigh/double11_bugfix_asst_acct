...
public class SellerRefundListAction implements Action{
	private final Log log =LogFactory.getLog(SellerRefundListAction.class);
	 ...
	 * 统计申请的退款总数
	 */
	private Long buyerRefundCount;
	public Long getBuyerRefundCount() {
		Map<String,Object> map = new HashMap<String,Object>();
		map.put("buyerId", CookieLoginInfo.getCookieLoginInfo().getMemberId()); //用户ID
		map.put("beginDate", DateFormatUtils.format(DateUtils.addMonths(new Date(), -3), "yyyyMMdd 00:00:00"));
		map.put("endDate", DateFormatUtils.format(new Date(), "yyyyMMdd HH:mm:ss"));
		buyerRefundCount = (long) refundApplyAO.loadRefundApplyInfoCount(map);
		return buyerRefundCount == null ? 0L : buyerRefundCount;
	}
	
	@AssistantPermission(target = "crm", action = "refund")
	public String execute() {
		if(log.isInfoEnabled()){
			log.info("Entry: SellerRefundListAction.execute() start");
		}
		...
		map.put("sellerId", CookieLoginInfo.getCookieLoginInfo().getMasterMemberId()); //用户ID
		
		if(paginator==null){
			paginator = new Paginator();
			if(StringUtils.isBlank(beginDate)){
				Date _beginDate = DateUtils.addMonths(new Date(), -1);
	...
	private void handleRefundList(Map<String,Object> map) {
		...
		if (refundList != null && refundList.size() > 0) {
			...
			for (RefundDO myrefund : refundList) {
				...
				Date outDate = refundBaseAO.getOvertimes(myrefund);
	...
	public SellerRefundListAction getAction(){
		return this;
	}
