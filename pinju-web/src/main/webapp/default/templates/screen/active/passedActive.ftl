...
<body>
	...
	<div class="pl-aleft">
		<ul class="pl-act">
			<li><a href="${base}/active/listActive.htm">可参与的活动</a></li>
			<li><a href="${base}/active/query.htm">我报名的活动</a></li>
			<li class="pl-here"><a href="${base}/active/query.htm?query.checkStatus=1">已参与的活动</a></li>
		</ul>
				...
				<#if (activeInfoList?size>0) >
					...
					<tr class="pl-actr">
						...
						<td>
							...
							<br />
							<span style="display:block; width:300px;">活动简介：<br />${(activeInfo.brief)!?html}</span>
						</td>
						<td><#if activeInfo.registType == 1>商品报名<#else>店铺报名</#if></td>
						<td>${(activeInfo.points)!} 积分</td>
						<td>
							<#if (flags[activeInfo_index])!true>
		...
		<div class="sel-ye" style="float: right">
			<form action="" method="post" name="pageForm" id="pageForm">
				...
				<input type="hidden" name="checkStatus" id="checkStatus" value="${(query.checkStatus)!0}">
			</form>
		</div>
	</div>
