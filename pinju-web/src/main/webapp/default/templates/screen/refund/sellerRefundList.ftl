...
<SCRIPT src="http://static.pinju.com/js/refund/sellerRefundList.js"></SCRIPT>
...
<div class="main062702">
  ...
  <a href="${base}/my/sell.htm" class="mr10">我是卖家 </a>&gt;
...
<ul class="t2 paddingbot5">
  <li style="padding:12px 0 0 8px">
		...
		<input type="text" name="beginDate" value="${beginDate!""}" style="width:160px" class="Wdate" id="beginDate" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})">
	 	到
	    <input type="text" name="endDate" id="endDate" style="width:160px" class="Wdate" value="${endDate!""}" onclick="WdatePicker({dateFmt:'yyyy-MM-dd HH:mm:ss'})">
  </li>
   <li style="padding:12px 0 0 8px">
		退款编号：
...
　　　<input type="button" value="搜索" class="btn-single-sgray" onclick="$('#form1').submit();" />
  </li></br>
</ul><!--成交信息结束 -->
<ul class="tab-nav"><li id="tg1" class="selected"><a href="${base}/refund/sellerRefundList.htm">我收到的退款(${paginator.items!"0"})</a></li><li id="tg2"><a href="${base}/refund/buyerRefundList.htm" >我申请的退款(${buyerRefundCount!"0"})</a></li></ul><!--状态信息结束 -->
...
<#if refundList?exists>
  ...
  <tr class="center">
    ...
    <td>
    	...
    	${timesOut.get(refund.id)?datetime}
    
    </td>
    <td><p class="hong">${refund.refundStateDisplay}</p></td>
    <td>
    	<#if refund.refundState==1>
    	<a href="sellerCheck.htm?id=${refund.id}" class="lan" target="_blank">查看</a>
    	<#elseif refund.refundState==2>
    	<a href="sellerViewWaitGoodsReturn.htm?id=${refund.id}" class="lan" target="_blank">查看</a>
    	<#elseif refund.refundState==3>
    	<a href="sellerViewConfirmGoods.htm?id=${refund.id}" class="lan" target="_blank">查看</a>
    	<#elseif refund.refundState==4>
    	<a href="sellerViewRefundClosed.htm?id=${refund.id}" class="lan" target="_blank">查看</a>
    	<#elseif refund.refundState==5>
    	<a href="sellerViewRefundSuccess.htm?id=${refund.id}" class="lan" target="_blank">查看</a>
    	<#elseif refund.refundState==6>
    	<a href="viewSellerReject.htm?id=${refund.id}" class="lan" target="_blank">查看</a>
    	<#elseif refund.refundState==7>
    	<a href="sellerViewRefundCustProcess.htm?id=${refund.id}" class="lan" target="_blank">查看</a>
    	<#elseif refund.refundState==8>
    	<a href="sellerViewRefundFail.htm?id=${refund.id}" class="lan" target="_blank">查看</a>
    	<#elseif refund.refundState==9>
    	<a href="sellerViewRefundProtocalAgree.htm?id=${refund.id}" class="lan" target="_blank">查看</a>
    	</#if>
    </td>
  </tr>
  </#list>
  ...
  <#else>
  		<tr><td colspan="7" align='center'> 没有找到符合条件的退款信息</td></tr>
</#if>
...
$('#seltState').val('${seltState!"all"}');
 ...
 <input type="hidden" value="seller-refund-management" id="my-page" />
