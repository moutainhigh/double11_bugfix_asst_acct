...
$.extend($.fn, {
  ...
  effectiveAndValidate : function( validateOptions, effectiveOptions ) {
    ...
    $.validator.addMethod("pattern2", function(value, element, parameter) {
        return parameter.test(value);
      }, 'format error');
    
    $.validator.addMethod("byterangelength", function(value, element, parameter) {
        var length = 0;
        for (var i = 0, k = value.length; i < k; i++) {
          length += (value.charCodeAt(i) < 0xff) ? 1 : 2;   
        }
        return this.optional(element) || ( length >= parameter[0] && length <= parameter[1] );
      }, $.validator.format("Please enter a value between {0} and {1} bytes long."));
    ...
    init : function() {
      ...
      $(this.form).find(this.settings.wrapSelector).each(function(i) {
        var elements = $(this).find('input, select, textarea')
                           ...
                           .not(me.settings.ignoreSelectors.join(', '))
                           .addClass(me.settings.inputNormalClass);
        me.settings.debug && window.console && console.info('init effective validate, %d element(s) is selected', elements.length);
      ...
      elements.each( function() {
        $.extend( this, {
          ...
          showMessage : function() {
            me.settings.debug && window.console && console.info('trigger showMessage event, element name: %s, message: %s, current class: %s', this.name, info, $(this).attr('class'));
            this.messageBox.show().removeClass(me.settings.message.errorClass).text(info || '');
          },
          showError : function(errorMessage) {
            me.settings.debug && window.console && console.info('trigger showError event, element name: %s, message: %s, current class: %s', this.name, errorMessage, $(this).attr('class'));
            this.messageBox.show().addClass(me.settings.message.errorClass).text(errorMessage);
          },
          hideMessage : function() {
            me.settings.debug && window.console && console.info('trigger hideMessage event, hide element name: %s message, current class: %s', this.name, $(this).attr('class'));
    ...
    bindEvent : function(elements, me) {
      ...
      elements.each(function(i) {
        ...
        $.each(me.settings.event, function(name, fn) {
          ...
          if (!!$(element).attr('ignore' + name)) {
//            $('#console').html($('#console').html() + $(element).attr('name') + ' : ' + name + ' : ' + !!$(element).attr('ignore' + name) + '<br />');
            me.settings.debug && window.console && console.info('bindEvent ignore %s event, name %s, ignore focus', name, $(element).attr('name'));
            return;
          }
