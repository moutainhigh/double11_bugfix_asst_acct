...
(function($) {
  ...
  var initDefaultOption = function() {
    for (var i = 0, k = linkages.length; i < k; i++) {
      $(linkages[i]).context.options[0] = makeOption(DEFAULT_VALUE, DEFAULT_CHOICE);
    }
  };

  var initDefaultValue = function(links) {
    if (!settings.defaultValue) {
      return;
    }
    links.unshift(ROOT_CODE);
    for (var i = 0, k = linkages.length; i < k; i++) {
      appendCode( linkages[i], links[i], links[i + 1], links[i + 1]);
    }
  };
  ...
  var selectOptions = function( code, defaultSelectedCode, selectedCode ) {
    ...
    while ( pcode == code) {
      ...
      opts.push( makeOption(value, data[offset][1], value == defaultSelectedCode, value == selectedCode) );
      if(offset == max) {
        break;
      }
      pcode = data[++offset][2];
  ...
  var makeOption = function( value, label, defaultSelected, selected ) {
    return new Option(label, value, !!defaultSelected, !!selected);
  };

  var appendCode = function(object, code, defaultSelectedCode, selectedCode  ) {
    var options = selectOptions( code, defaultSelectedCode, selectedCode  );
    appendOptions( object, options );
  };

  var appendOptions = function(object, options) {
    if (!options || options.length == 0) {
      return;
    }
    var obj = $(object).context;
    for (var i = 0, k = options.length; i < k; i++) {
      obj.options.add(options[i]);
    }
  };
  ...
  var Linkage = function() {
    ...
    return {
      ...
      init : function(obj) {
        ...
        initLinkages();
        if (!settings.defaultValue) {
          appendCode( linkages[0], ROOT_CODE );
        } else {
          initDefaultValue(this.getLinks());
        }
        return this;
      },
       ...
       * 设置初始值
       */
      getLinks : function(arg) {
        if(!arg && !settings.defaultValue) {
          return [];
        }
        var links = [], v = arg || settings.defaultValue, n = 100;
        while(v > 0) {
          if(v % n != 0) {
            links.unshift(v);
          }
      ...
      setValue : function() {        
        ...
        var links = this.getLinks(arguments[0]);
