...
PJ.DOMAIN = 'pinju.com';
PJ.COOKIE_DOMAIN = '.' + PJ.DOMAIN;
PJ.ROOT = 'http://www.' + PJ.DOMAIN;
PJ.ROOT_ENCODED = encodeURIComponent(PJ.ROOT);
...
Person : {
    ...
    $('#form-input :text, #form-input :password, #form-input select, #form-input textarea, #form-input :checkbox').each(function() {
      ...
      function checkRequired(elements) {
        ...
        elements.each(function() {
          var t = $(this);
          if(t.attr('required') && pinju.Person.trim(t.val()).length == 0) {
            ok = false;
          }
        });
        return ok;
  ...
  defaultRadioValue : function(selector, value, defaultValue) {
    if(!value || pinju.Person.trim(value).length == 0) {
      value = defaultValue;
    }
