...
KindEditor.plugin('image', function(K) {
	var self = this, name = 'image',
		...
		readonlyImageUrl = K.undef(self.readonlyImageUrl, false),
	...
	self.plugin.imageDialog = function(options) {
		...
		var html = [
			...
			'<input type="text" id="keUrl" class="ke-input-text" name="url" value="" style="width:200px;" ' + (readonlyImageUrl ? ' readonly="true"' : '') + '/> &nbsp;',
			...
			'<input type="button" class="ke-upload-button" value="' + lang.viewLocal + '" />',
			'</div>',
			'</div>',
			//size
			'<div class="ke-dialog-row">',
		...
		var uploadbutton = K.uploadbutton({
			...
			afterError : function(str) {
				...
				// <h3>很抱歉，您上传的文件超过最大限制3M！</h3>
				var msg, match;
				if ((match = /<h3>([\s\S]+?)<\/h3>/.exec(str))) {
					alert(K.trim(match[1]));
					return;
				}
				self.errorDialog(str);
			}
		});
